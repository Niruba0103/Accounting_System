...
const ReceiptEntry = () => {
  ...
  const fetchSalesInvoices = async () => {
    try {
      ...
      const onlySales = response.data.filter(
        (invoice) =>
          invoice.invoice_type === 'SALES' &&
          Number(invoice.balance_amount) > 0
      );
      setSalesInvoices(onlySales);
    } catch (error) {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'invoice_id') {
      ...
      setFormData({
        ...
        reason: invoice
          ? `Receipt against invoice ${invoice.invoice_no}`
          : formData.reason
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.post('/invoices/receipt', {
        ...formData,
      ...
      console.error('Receipt entry error:', error);
      alert(error.response?.data?.message || 'Error recording receipt');
    }
  };
